...
export const TimelineClip: React.FC<TimelineClipProps> = ({
  ...
  const getBorderColor = () => {
    if (isSelected) return 'border-blue-300';
    if (isHovered) return 'border-blue-200';
    return 'border-blue-700';
  };

  return (
    <div
      className={`absolute cursor-pointer rounded border-2 transition-all duration-200 ${getClipColor()} ${getBorderColor()}`}
      style={{
        left: `${left}px`,
        width: `${width}px`,
        top: `${top}px`,
      ...
      title={`Clip: ${clipDuration.toFixed(1)}s (${clip.startTime.toFixed(1)}s - ${clip.endTime.toFixed(1)}s)`}
    >
      {/* Clip content */}
      ...
      {/* Selection indicator */}
      {isSelected && (
        <div className="absolute -top-1 -bottom-1 -left-1 -right-1 border-2 border-blue-300 rounded pointer-events-none" />
      )}
    </div>
  );
};
